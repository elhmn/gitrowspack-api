...
module.exports = class Gitrows {
  ...
  drop(path) {
    ...
    if (self.ns == "github")
      return self
        .pull(path)
        .then((d) => self.push(path, null, d.sha, "DELETE"));
  ...
  get(path, query, method) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      return self
        ._pullOrFetch(url, method)
        .then((t) => {
          let data = Gitrows._parse(t, self.type);
          if (data) self._meta.count = { total: data.length || 0 };
  ...
  getDatabases(repo, target = null) {
    ...
    return new Promise(function (resolve, reject) {
      try {
        fetch(
          `https://api.github.com/repos/${self.owner}/${repo}/git/trees/${self.branch}?recursive=1`
        )
          .then((res) => res.json())
          .then((data) => {
            if (target !== null)
              resolve(
                data.tree.filter((d) => d.path === target).map((d) => d.sha)[0]
              );
            else
              resolve(
                data.tree.filter((d) => d.type === "tree").map((d) => d.path)
              );
          });
      } catch (err) {
        console.log(err);
  ...
  getCollections(repo, collection) {
    ...
    return new Promise(async function (resolve, reject) {
      try {
        ...
        fetch(
          `https://api.github.com/repos/${self.owner}/${repo}/git/trees/${tree_sha}`
        )
          .then((res) => res.json())
          .then((data) => {
            resolve(
              data.tree
                ...
                .map((d) => {
                  return { collection: d.path, size: d.size };
                })
            );
          });
      } catch (e) {
  ...
  options(obj) {
    ...
    for (let key in obj) {
      if (allowed.includes(key) && typeof obj[key] !== "undefined")
        this[key] = obj[key];
    }
    return self;
  }
  async test(path, constraint = {}) {
    ...
    if (!result.fragment) {
      const file = await this._isRepoFile(path)
        .then((r) => r)
        .catch((e) => e);
    ...
    Object.keys(constraint).forEach((item, i) => {
      if (result[item] !== constraint[item]) {
        ...
        if (result.message && Array.isArray(result.message.description))
          result.message.description.push(
            `Constraint Violation - ${item} must not be ${result[item]}`
          );
        else
          result = {
            ...result,
        ...
        return r.json().then((r) => {
          ...
          if (!!grouped) {
            ...
            Object.keys(repos).forEach((item, i) => {
              repos[item]["repos"] = r
                .filter((f) => f.permissions.push && f.owner.login == item)
                .map((r) => `@${ns}/${r.full_name}`);
            });
          ...
          repos = r
            .filter((f) => f.permissions.push)
            .map((r) => `@${ns}/${r.full_name}`);
          if (typeof owner != "undefined")
            repos = repos.filter((r) => ~r.indexOf(`@${ns}/${owner}/`));
          self._cache[hash]["as"] = repos;
          self._cache[hash]["at"] = Date.now();
          return repos;
        });
  ...
  _isRepoFile(path) {
    ...
    const test = GitPath.parse(path) || {};
    return self
      ._listRepoContents(path)
      .then(
        (c) =>
          c.findIndex(
            (item) => test.path.toLowerCase() === item.toLowerCase()
          ) > -1
      )
      .catch((e) => e);
  }
      ...
      switch (type.toLowerCase()) {
        case "csv":
          return Array.isArray(obj)
            ? CSV.stringify(obj, Object.assign({ header: true }, options))
            : null;
          break;
        case "yaml":
